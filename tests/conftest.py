...
def two_users(client, app):
    ...
    client.post('/register', json={'username': 'alice', 'nickname': 'alice_n', 'password': 'p1'})
    client.post('/register', json={'username': 'bob', 'nickname': 'bob_n', 'password': 'p2'})

    alice_login = client.post('/login', json={'username': 'alice', 'password': 'p1'}).get_json()
    bob_login = client.post('/login', json={'username': 'bob', 'password': 'p2'}).get_json()
