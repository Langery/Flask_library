...
class TestRegisterLogin:
    def test_register_then_login_succeeds(self, client):
        r = client.post('/register', json={'username': 'u1', 'nickname': 'u1n', 'password': 'pw'})
        assert r.status_code == 200
    ...
    def test_register_missing_fields_returns_400(self, client):
        r = client.post('/register', json={'username': 'only_name'})
        assert r.status_code == 400

    def test_login_wrong_password_returns_backdata_false(self, client):
        client.post('/register', json={'username': 'u2', 'nickname': 'u2n', 'password': 'pw'})
        r = client.post('/login', json={'username': 'u2', 'password': 'WRONG'})
