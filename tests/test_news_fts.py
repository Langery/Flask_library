...
class TestNewsFtsSearch:
    ...
    def test_fts_finds_single_word(self, client, auth_headers):
        ...
        r = client.get('/api/news/list?q=News&_limit=5', headers=auth_headers('alice'))
    ...
    def test_fts_multi_token_and_semantics(self, client, auth_headers):
        ...
        r = client.get('/api/news/list?q=News+11&_limit=20', headers=auth_headers('alice'))
        assert r.status_code == 200
        items = r.get_json()['data']['items']
    ...
    def test_fts_no_match_returns_empty(self, client, auth_headers):
        ...
        r = client.get('/api/news/list?q=xyzzy_no_such_token', headers=auth_headers('alice'))
        assert r.status_code == 200
    ...
    def test_fts_pure_special_chars_falls_back_to_all(self, client, auth_headers):
        ...
        r = client.get('/api/news/list?q=!@%23%24&_limit=5', headers=auth_headers('alice'))
        assert r.status_code == 200
        items = r.get_json()['data']['items']
        # 应返回种子数据,不是空
        assert len(items) == 5
    ...
    def test_fts_trigger_syncs_direct_insert(self, client, auth_headers):
        ...
        r = client.get('/api/news/list?q=unique_fts_marker', headers=auth_headers('alice'))
        assert r.status_code == 200
        items = r.get_json()['data']['items']
