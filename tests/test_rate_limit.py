...
class TestLoginRateLimit:
    ...
    def test_5_logins_then_6th_returns_429(self, client, monkeypatch):
        ...
        # 先注册一个用户
        client.post('/register', json={'username': 'rl1', 'nickname': 'rl1n', 'password': 'pw'})
        ...
        # 5 次合法(密码错也行,关键是触发计数)
        for i in range(5):
            r = client.post('/login', json={'username': 'rl1', 'password': 'WRONG'})
    ...
    def test_rate_limit_blocks_even_correct_password(self, client):
        """触发限速后,即使密码正确也应被拒。"""
        client.post('/register', json={'username': 'rl2', 'nickname': 'rl2n', 'password': 'correct_pw'})

        # 触发限速(6 次)
        for _ in range(6):
            client.post('/login', json={'username': 'rl2', 'password': 'WRONG'})

        # 正确密码也应 429
        r = client.post('/login', json={'username': 'rl2', 'password': 'correct_pw'})
    ...
    def test_other_endpoints_not_rate_limited(self, client, auth_headers):
        ...
        r = client.get('/news/list?_page=1&_limit=3', headers=auth_headers('alice'))
